"""Geo module tests: mock geodata/workout importer research targets (#113)."""
...
from ios_research.targets.geo import GEO_TARGETS
...
GEO_IDS = sorted(GEO_TARGETS)


@pytest.mark.parametrize("target_id", GEO_IDS)
def test_geo_targets_registered(target_id):
    ids = [t["id"] for t in list_targets()]
    assert target_id in ids


@pytest.mark.parametrize("target_id", GEO_IDS)
def test_geo_seed_is_accepted(target_id):
...
@pytest.mark.parametrize("target_id", GEO_IDS)
def test_geo_rejects_foreign_input(target_id):
    assert create(target_id).execute(b"not-geodata-bytes").outcome == Outcome.REJECTED


@pytest.mark.parametrize("target_id", GEO_IDS)
def test_geo_oob_read_on_oversized_declared_length(target_id):
    target = create(target_id)
    data = target.magic + (0x00FF).to_bytes(2, "big") + bytes([1, 8]) + b"x"
...
@pytest.mark.parametrize("target_id", GEO_IDS)
def test_geo_zero_point_scale_is_integer_error(target_id):
    target = create(target_id)
    payload = b"body"
...
@pytest.mark.parametrize("target_id", GEO_IDS)
def test_geo_null_geometry_kind_is_null_dereference(target_id):
    target = create(target_id)
    payload = b"body"
    data = target.magic + len(payload).to_bytes(2, "big") + bytes([0x00, 8]) + payload
    res = target.execute(data)
    assert res.outcome == Outcome.CRASH
...
@pytest.mark.parametrize("target_id", GEO_IDS)
def test_geo_dead_marker_is_use_after_free(target_id):
    target = create(target_id)
    payload = b"\xde\xadbody"
    data = target.magic + len(payload).to_bytes(2, "big") + bytes([1, 8]) + payload
    res = target.execute(data)
    assert res.outcome == Outcome.CRASH
...
@pytest.mark.parametrize("target_id", GEO_IDS)
def test_geo_timeout_on_oversized_declared_length(target_id):
    target = create(target_id)
    # declared >= 0xF000 takes the timeout path (checked before OOB).
    data = target.magic + (0xF000).to_bytes(2, "big") + bytes([1, 8]) + b"x"
    assert target.execute(data).outcome == Outcome.TIMEOUT


@pytest.mark.parametrize("target_id", GEO_IDS)
def test_geo_type_confusion_and_assertion_paths(target_id):
    target = create(target_id)
    payload = b"body"
    confused = target.magic + len(payload).to_bytes(2, "big") + bytes([0xC0, 8]) + payload
    assert target.execute(confused).diagnostics.classification_hint == "TYPE_CONFUSION"
    asserted = target.magic + len(payload).to_bytes(2, "big") + bytes([0x7E, 8]) + payload
    assert target.execute(asserted).diagnostics.classification_hint == "ASSERTION"


def test_geo_structure_mutate_is_format_aware():
    target = create("geo:gpx")
    rng = mutation.rng_for(1, 1)
    mutated = target.structure_mutate(target.seeds()[0], rng)
    assert mutated.startswith(target.magic)


def test_geo_diagnostics_are_deterministic():
    target = create("geo:tile-proto")
    data = target.magic + (0x00FF).to_bytes(2, "big") + bytes([1, 8]) + b"x"
...
def test_geo_fuzz_finds_multiple_classifications(workspace):
    exp = ExperimentStore(workspace).create(
        target="geo:geojson", device="mock:device", os_version="17.0",
        config_hash="cfg_x", seed=3)
    cs = CorpusStore(workspace)
    corpus = cs.create("geo")
    for s in create("geo:geojson").seeds():
        cs.add_bytes(corpus, s, origin="seed")
    engine = FuzzEngine(workspace)
    session = engine.create(experiment_id=exp.id, target="geo:geojson",
