"""Wi-Fi Aware module tests: mock Wi-Fi Aware frame-parser research targets."""
...
from ios_research.targets.wifiaware import WIFIAWARE_TARGETS
...
WIFIAWARE_IDS = sorted(WIFIAWARE_TARGETS)


@pytest.mark.parametrize("target_id", WIFIAWARE_IDS)
def test_wifiaware_targets_registered(target_id):
    ids = [t["id"] for t in list_targets()]
    assert target_id in ids


@pytest.mark.parametrize("target_id", WIFIAWARE_IDS)
def test_wifiaware_seed_is_accepted(target_id):
...
@pytest.mark.parametrize("target_id", WIFIAWARE_IDS)
def test_wifiaware_rejects_foreign_input(target_id):
    assert create(target_id).execute(
        b"not-wifi-aware-frames").outcome == Outcome.REJECTED


@pytest.mark.parametrize("target_id", WIFIAWARE_IDS)
def test_wifiaware_oob_read_on_oversized_declared_length(target_id):
    target = create(target_id)
    data = target.magic + (0x00FF).to_bytes(2, "big") + bytes([1, 3]) + b"x"
...
@pytest.mark.parametrize("target_id", WIFIAWARE_IDS)
def test_wifiaware_zero_tlv_count_is_integer_error(target_id):
    target = create(target_id)
    payload = b"body"
...
@pytest.mark.parametrize("target_id", WIFIAWARE_IDS)
def test_wifiaware_reclaim_marker_is_use_after_free(target_id):
    target = create(target_id)
    payload = b"\xde\xadbody"
    data = target.magic + len(payload).to_bytes(2, "big") + bytes([1, 3]) + payload
    res = target.execute(data)
    assert res.outcome == Outcome.CRASH
...
@pytest.mark.parametrize("target_id", WIFIAWARE_IDS)
def test_wifiaware_timeout_on_oversized_declared_length(target_id):
    target = create(target_id)
    # declared >= 0xF000 takes the timeout path (checked before OOB).
    data = target.magic + (0xF000).to_bytes(2, "big") + bytes([1, 3]) + b"x"
    assert target.execute(data).outcome == Outcome.TIMEOUT


@pytest.mark.parametrize("target_id", WIFIAWARE_IDS)
def test_wifiaware_type_confusion_and_assertion_paths(target_id):
    target = create(target_id)
    payload = b"body"
    confused = target.magic + len(payload).to_bytes(2, "big") \
        + bytes([0xC0, 3]) + payload
    assert target.execute(confused).diagnostics.classification_hint == "TYPE_CONFUSION"
    asserted = target.magic + len(payload).to_bytes(2, "big") \
        + bytes([0x7E, 3]) + payload
    assert target.execute(asserted).diagnostics.classification_hint == "ASSERTION"


def test_wifiaware_structure_mutate_is_format_aware():
    target = create("wifiaware:publish")
    rng = mutation.rng_for(1, 1)
    mutated = target.structure_mutate(target.seeds()[0], rng)
    assert mutated.startswith(target.magic)


def test_wifiaware_diagnostics_are_deterministic():
    target = create("wifiaware:datapath")
    data = target.magic + (0x00FF).to_bytes(2, "big") + bytes([1, 3]) + b"x"
...
def test_wifiaware_fuzz_finds_multiple_classifications(workspace):
    exp = ExperimentStore(workspace).create(
        target="wifiaware:publish", device="mock:device", os_version="17.0",
        config_hash="cfg_x", seed=3)
    cs = CorpusStore(workspace)
    corpus = cs.create("wifiaware")
    for s in create("wifiaware:publish").seeds():
        cs.add_bytes(corpus, s, origin="seed")
    engine = FuzzEngine(workspace)
    session = engine.create(experiment_id=exp.id, target="wifiaware:publish",
