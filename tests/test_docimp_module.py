"""Document-importer module tests: mock docimp structure-parser targets."""
...
from ios_research.targets.docimp import DOCIMP_TARGETS
...
DOCIMP_IDS = sorted(DOCIMP_TARGETS)


@pytest.mark.parametrize("target_id", DOCIMP_IDS)
def test_docimp_targets_registered(target_id):
    ids = [t["id"] for t in list_targets()]
    assert target_id in ids


@pytest.mark.parametrize("target_id", DOCIMP_IDS)
def test_docimp_seed_is_accepted(target_id):
...
@pytest.mark.parametrize("target_id", DOCIMP_IDS)
def test_docimp_rejects_foreign_input(target_id):
    assert create(target_id).execute(b"not-document-bytes").outcome == Outcome.REJECTED


@pytest.mark.parametrize("target_id", DOCIMP_IDS)
def test_docimp_oob_read_on_oversized_declared_length(target_id):
    target = create(target_id)
...
@pytest.mark.parametrize("target_id", DOCIMP_IDS)
def test_docimp_part_class_zero_is_null_dereference(target_id):
    target = create(target_id)
    payload = b"body"
...
@pytest.mark.parametrize("target_id", DOCIMP_IDS)
def test_docimp_table_flag_is_integer_error(target_id):
    target = create(target_id)
    payload = b"body"
    data = target.magic + len(payload).to_bytes(2, "big") + bytes([1, 0x04]) + payload
    res = target.execute(data)
    assert res.outcome == Outcome.CRASH
...
@pytest.mark.parametrize("target_id", DOCIMP_IDS)
def test_docimp_dead_marker_is_use_after_free(target_id):
    target = create(target_id)
...
@pytest.mark.parametrize("target_id", DOCIMP_IDS)
def test_docimp_timeout_on_oversized_declared_length(target_id):
    target = create(target_id)
...
@pytest.mark.parametrize("target_id", DOCIMP_IDS)
def test_docimp_type_confusion_and_assertion_paths(target_id):
    target = create(target_id)
    payload = b"body"
...
def test_docimp_structure_mutate_is_format_aware():
    target = create("docimp:zip-archive")
    rng = mutation.rng_for(1, 1)
    mutated = target.structure_mutate(target.seeds()[0], rng)
    assert mutated.startswith(target.magic)


def test_docimp_diagnostics_are_deterministic():
    target = create("docimp:pdfform")
    data = target.magic + (0x00FF).to_bytes(2, "big") + bytes([1, 0]) + b"x"
...
def test_docimp_fuzz_finds_multiple_classifications(workspace):
    exp = ExperimentStore(workspace).create(
        target="docimp:ooxml-part", device="mock:device", os_version="17.0",
        config_hash="cfg_x", seed=3)
    cs = CorpusStore(workspace)
    corpus = cs.create("docimp")
    for s in create("docimp:ooxml-part").seeds():
        cs.add_bytes(corpus, s, origin="seed")
    engine = FuzzEngine(workspace)
    session = engine.create(experiment_id=exp.id, target="docimp:ooxml-part",
