"""Signed-document module tests: mock signed-identity-document targets."""
...
from ios_research.targets.signeddoc import SIGNEDDOC_TARGETS
...
SIGNEDDOC_IDS = sorted(SIGNEDDOC_TARGETS)


@pytest.mark.parametrize("target_id", SIGNEDDOC_IDS)
def test_signeddoc_targets_registered(target_id):
    ids = [t["id"] for t in list_targets()]
    assert target_id in ids


@pytest.mark.parametrize("target_id", SIGNEDDOC_IDS)
def test_signeddoc_seed_is_accepted(target_id):
...
@pytest.mark.parametrize("target_id", SIGNEDDOC_IDS)
def test_signeddoc_rejects_foreign_input(target_id):
    assert create(target_id).execute(b"not-signed-bytes").outcome == Outcome.REJECTED


@pytest.mark.parametrize("target_id", SIGNEDDOC_IDS)
def test_signeddoc_oob_read_on_oversized_declared_length(target_id):
    target = create(target_id)
...
@pytest.mark.parametrize("target_id", SIGNEDDOC_IDS)
def test_signeddoc_class_zero_is_null_dereference(target_id):
    target = create(target_id)
    payload = b"body"
...
@pytest.mark.parametrize("target_id", SIGNEDDOC_IDS)
def test_signeddoc_indefinite_length_flag_is_integer_error(target_id):
    target = create(target_id)
    payload = b"body"
    data = target.magic + len(payload).to_bytes(2, "big") + bytes([1, 0x08]) + payload
    res = target.execute(data)
    assert res.outcome == Outcome.CRASH
...
@pytest.mark.parametrize("target_id", SIGNEDDOC_IDS)
def test_signeddoc_released_set_marker_is_use_after_free(target_id):
    target = create(target_id)
...
@pytest.mark.parametrize("target_id", SIGNEDDOC_IDS)
def test_signeddoc_timeout_on_oversized_declared_length(target_id):
    target = create(target_id)
...
@pytest.mark.parametrize("target_id", SIGNEDDOC_IDS)
def test_signeddoc_type_confusion_and_assertion_paths(target_id):
    target = create(target_id)
    payload = b"body"
...
def test_signeddoc_structure_mutate_is_format_aware():
    target = create("signeddoc:profile")
    rng = mutation.rng_for(1, 1)
    mutated = target.structure_mutate(target.seeds()[0], rng)
    assert mutated.startswith(target.magic)


def test_signeddoc_diagnostics_are_deterministic():
    target = create("signeddoc:receipt")
    data = target.magic + (0x00FF).to_bytes(2, "big") + bytes([1, 0]) + b"x"
...
def test_signeddoc_fuzz_finds_multiple_classifications(workspace):
    exp = ExperimentStore(workspace).create(
        target="signeddoc:pkpass", device="mock:device", os_version="17.0",
        config_hash="cfg_x", seed=3)
    cs = CorpusStore(workspace)
    corpus = cs.create("signeddoc")
    for s in create("signeddoc:pkpass").seeds():
        cs.add_bytes(corpus, s, origin="seed")
    engine = FuzzEngine(workspace)
    session = engine.create(experiment_id=exp.id, target="signeddoc:pkpass",
