"""`ios-research target audio` — list and inspect audio targets.

Installs itself as a subcommand of ``target`` via the target command's
extension hook.
"""

from __future__ import annotations

from .. import targets
from ..errors import NotFoundError
from ..output import Result
from ..targets.audio import AUDIO_TARGETS
from . import target_cmd


def _install(sub, parent) -> None:
    p_audio = sub.add_parser("audio", parents=[parent],
                             help="audio-processing research targets")
    asub = p_audio.add_subparsers(dest="audio_action", metavar="<action>")

    p_list = asub.add_parser("list", parents=[parent], help="list audio targets")
    p_list.set_defaults(func=cmd_list)

    p_inspect = asub.add_parser("inspect", parents=[parent],
                                help="inspect an audio target")
    p_inspect.add_argument("format", help="wav|mp3|aac|alac or a full id")
    p_inspect.set_defaults(func=cmd_inspect)

    p_audio.set_defaults(func=cmd_list)


def register(subparsers, parent) -> None:
    # Register the installer with the target command group.
    target_cmd.add_subcommand(_install)


def _resolve(fmt: str) -> str:
    if fmt in AUDIO_TARGETS:
        return fmt
    candidate = f"audio:{fmt}"
    if candidate in AUDIO_TARGETS:
        return candidate
    raise NotFoundError(f"unknown audio format '{fmt}'")


def cmd_list(ctx, args) -> Result:
    items = [targets.create(tid).describe() for tid in sorted(AUDIO_TARGETS)]
    return Result(command="target audio list",
                  data={"targets": items, "count": len(items)},
                  human=lambda d: "\n".join(
                      f"{t['id']:14} {', '.join(t['formats'])}" for t in d["targets"]))


def cmd_inspect(ctx, args) -> Result:
    target_id = _resolve(args.format)
    target = targets.create(target_id)
    d = target.describe()
    d["seed_count"] = len(target.seeds())
    return Result(command="target audio inspect", data={"target": d})
