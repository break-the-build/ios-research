/* P4-LIVENESS probe: IOTimeSyncgPTPManager externalMethod liveness.
...
int main(int argc, char **argv) {
    ...
    printf("== P4-LIVENESS probe_mgr uid=%d euid=%d pid=%d sel=0x%llx ==\n",
           getuid(), geteuid(), getpid(), sel);

    CFDictionaryRef matching = IOServiceMatching("IOTimeSyncgPTPManager");
    if (!matching) {
    ...
    if (okr == KERN_SUCCESS && conn != MACH_PORT_NULL) {
        io_name_t cls = {0};
        kern_return_t gkr = IOObjectGetClass((io_object_t)conn, cls);
        printf("IOObjectGetClass(conn) kr=0x%x (%s) name=%s\n",
               gkr, mach_error_string(gkr), cls);
